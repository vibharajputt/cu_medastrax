...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    mediaQuery.addEventListener('change', _updateVal);
    return () => mediaQuery.removeEventListener('change', _updateVal);
  }, [theme]);
  ...
  const getUserRoleLabel = () => {
    if (activeProfile) return activeProfile.relation;
    const userType = localStorage.getItem('user_type');
    if (userType === 'FACULTY') return 'FACULTY';
    if (userType === 'PATIENT' || user?.role === 'PATIENT') return 'STUDENT';
    return user?.role;
  };
  ...
  if (
    ...
    location.pathname.startsWith('/lab/dashboard') ||
    location.pathname.startsWith('/hospital/dashboard') ||
    location.pathname.startsWith('/admin/dashboard') ||
    location.pathname.startsWith('/my-prescriptions') ||
    ...
    location.pathname.startsWith('/wellness-center') ||
    location.pathname.startsWith('/vaccinations') ||
    location.pathname.startsWith('/medical-leave') ||
    location.pathname.startsWith('/analytics') ||
    location.pathname.startsWith('/medicine-trends') ||
    location.pathname.startsWith('/wellness-score') ||
    location.pathname.startsWith('/refer-a-student') ||
    location.pathname.startsWith('/symptom-checker') ||
    location.pathname.startsWith('/student-health-portal') ||
    location.pathname.startsWith('/health-map') ||
            ...
            {navLinks.map((link, index) => (
              link.path === 'notifications' ? (
                ...
                <Link
                  key={`${link.path}-${index}`}
                  to={link.path}
                  className={`nav-link ${isLinkActive(link.path) ? 'active' : ''} ${link.path === '/care-plan' ? 'nav-care-plan-link' : ''} ${link.label === 'Home' ? 'nav-home-pill' : ''}`}
                  onClick={(e) => {
                ...
                >
                  ...
                  {showProfileDropdown && (
                    <div className="profile-dropdown-menu">
                      ...
                      <button className="profile-dropdown-item" onClick={() => { setShowProfileDropdown(false); navigate('/admin/dashboard'); }}>
                        <FiShield className="item-icon" style={{ color: '#0d9488' }} /> Admin Portal
                      </button>
